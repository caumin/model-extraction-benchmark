...
- Matrix-generation policy (`generate_configs.py`) uses setup-level unified substitute defaults:
  ...
  - **SET-C1 (`substitute.arch=xie2019`)**: `batch=256`, `optimizer=sgd(lr=0.1,momentum=0.9,wd=5e-4)`, `scheduler=multistep([0.5,0.75],gamma=0.1)`, `max_epochs=90`, `patience=90` with best-checkpoint selection by validation loss and no effective early stopping.
