#!/usr/bin/env python3
"""Runner for SET-C1 DFME generator-LR sweep (5 jobs)."""
...
CONFIGS = [
    "SET-C1_dfme_explore_lrG_1e-5_5m_seed1",
    "SET-C1_dfme_explore_lrG_1e-5_5m_seed2",
    "SET-C1_dfme_explore_lrG_1e-6_5m_seed0",
    "SET-C1_dfme_explore_lrG_3e-6_5m_seed0",
    "SET-C1_dfme_explore_lrG_3e-5_5m_seed0",
]
...
def build_queue() -> List[Job]:
    return [Job(n) for n in CONFIGS if (CFG_DIR / f"{n}.yaml").exists()]
...
def main() -> int:
    queue = build_queue()
    log(f"==== SET-C1 DFME gen-sweep: {len(queue)} jobs ====")
