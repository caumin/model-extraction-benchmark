#!/usr/bin/env python3
"""Runner for SET-C1 DFME failure-mode exploration (8 jobs, seed 0, 5M budget).

Each DFME run reserves ~38 GB peak on this GPU. The box currently has
another user's ~50 GB job running, so cap concurrency at 2 to stay under
~130 GB committed.
"""
...
MARGIN_GB = 4
POLL_INTERVAL_S = 60
LAUNCH_INTERVAL_S = 60

CONFIGS = [
    "SET-C1_dfme_explore_baseline_5m_seed0",
    "SET-C1_dfme_explore_sgd_lr1e-2_5m_seed0",
    "SET-C1_dfme_explore_sgd_lr1e-3_5m_seed0",
    "SET-C1_dfme_explore_adamw_lr1e-3_5m_seed0",
    "SET-C1_dfme_explore_lrG_1e-3_5m_seed0",
    "SET-C1_dfme_explore_lrG_1e-5_5m_seed0",
    "SET-C1_dfme_explore_ng5_5m_seed0",
    "SET-C1_dfme_explore_resnet18_5m_seed0",
]
...
def main() -> int:
    queue = build_queue()
    log(f"==== SET-C1 DFME explore: {len(queue)} jobs ====")
    ...
    while queue or running:
        ...
        committed = sum(j.vram_gb for j, _ in running) + 40 * len(ext)
        for j in list(queue):
            if already_done(j):
            ...
            if len(running) >= MAX_CONCURRENT: break
