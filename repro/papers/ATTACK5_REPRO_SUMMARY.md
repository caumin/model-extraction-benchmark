...
|---|---|---|---|
| DFME | `repro/papers/2021_truong_dfme/results/reproduced_metrics.json` | 100k..20M | practical reproduction success: `20M acc_gt=0.8710` vs paper `0.8810` (`-1.00pp`, tolerance-edge) |
| DFMS-HL | `repro/papers/2022_sanyal_dfms/results/reproduced_metrics.json` | smoke 200..2000 | full rerun required (target budget row still missing) |
| MAZE | `repro/papers/2021_kariyappa_maze/results/reproduced_metrics.json` | 1k..20M | original MAZE-paper target (`30M/0.8985`) unmet, but `20M` behavior (`final 0.4009`, `peak 0.4454`) is close to follow-up DFME-paper MAZE reference (`45.6%`) |
