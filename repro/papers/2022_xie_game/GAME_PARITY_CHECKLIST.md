...
## V. 2026-02-23 Refactor Prep Slice

- [x] Non-parity dead code cleanup completed outside GAME core logic:
  - `mebench/attackers/copycatcnn.py` legacy LR helper removed.
  - `mebench/utils/chunked.py` dead helpers removed (`create_chunked_dataloader`, `chunked_inference`).
  - `mebench/utils/validation.py` unused `_global_validator` removed.
- [x] P2 safe-deprecation policy applied for chunked API surface:
  - `ChunkedProcessor.stream_dataset_items` and `ChunkedProcessor.accumulate_chunks` now deprecated with removal target `v1.4`.
- [x] GAME refactor plan/checklist added:
  - `docs/design/GAME_REFACTOR_ULTRAWORK_PLAN.md`
- [x] Repro runbook/todo added:
  - `repro/papers/2022_xie_game/ULTRAWORK_RUNBOOK.md`
  - `repro/papers/2022_xie_game/TODO_ULTRAWORK.md`
- [ ] Next execution batch:
  - run pair-1 and pair-2 commands from `ULTRAWORK_RUNBOOK.md` after current game.py refactor validation snapshot is locked.
