...
| Victim input scaling | no wrapper conversion at victim boundary | `victim.input_scale_mode` / `input_scale_mode` | `unit` in full+smoke+victim train/eval configs |
...
| Oracle query scaling | proxy/synthetic queries routed in tanh space | `attack.query_input_scale_mode` | `tanh` in full+smoke configs |
