...
4. Matrix substitute policy is setup-level unified:
   ...
   - SET-C1 (`xie2019`) uses one fixed-length substitute profile across attacks: `sgd(lr=0.1,momentum=0.9,wd=5e-4)`, `batch=256`, `multistep([0.5,0.75],gamma=0.1)`, `max_epochs=90`, `patience=90`, with best-checkpoint selection by validation loss.
