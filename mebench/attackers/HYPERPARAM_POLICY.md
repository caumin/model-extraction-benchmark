...
| `blackbox_dissector` | Core defaults from official script: `batch_size=128`, `lr=0.02`, `momentum=0.9`, `train_epochs=200` | Selection batch, iterative budget schedule, erase ratio exploration | **Aligned (SET-B policy)** via config alignment + code defaults (`official_repo_clones/blackbox-dissector/attack.py:44-55`, `mebench/attackers/blackbox_dissector.py:456-463`) |
...
| `dfme` | Official defaults: `batch=256`, `g_iter=1`, `d_iter=5`, `lr_S=0.1`, `lr_G=1e-4` | Eval cadence, checkpointing, scheduler granularity | **Aligned (SET-B policy)** with optimizer/lr-per-sample path (`official_repo_clones/datafree-model-extraction/dfme/train.py:182-189`, `mebench/attackers/dfme.py:26-33`, `mebench/attackers/dfme.py:67-73`) |
...
| `disguide` | Official CIFAR script: `batch=256`, `lr_S=0.03`, `lr_G=1e-4`, replay-style loop | Replay size, grayscale cadence, eval interval | **Aligned (SET-B policy)** for optimizer/lr-per-sample with same family (`official_repo_clones/disguide/run_cifar-10.sh:13`, `official_repo_clones/disguide/run_cifar-10.sh:22`, `mebench/attackers/disguide.py:145-154`) |
| `ds` | Official DS defaults: `lr_S=0.3`, `lr_G=1e-4`, `num_students>=2` | Logging/eval intervals, strict budget stepping | **Aligned (SET-B policy)** and code-level constraints present (`official_repo_clones/dual_students/run_cifar_ds.sh:11-14`, `mebench/attackers/ds.py:51-56`) |
